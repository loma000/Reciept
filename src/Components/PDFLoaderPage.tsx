...
const s = StyleSheet.create({
  ...
  sigRow: { flexDirection: "row", justifyContent: "flex-end", marginTop: 16 },
...
export const RentReceiptDoc: React.FC<{ data?: ReceiptData }> = ({
  data = {},
}) => {
  const {
    name = "",
    ...
    electricMeterPrev: eletricMeterPrev = "",
    electricMeterUsed: eletricMeterUsed = "",
    items = {},
    Receiver = "",
    bankName = "",
    accountName = "",
      ...
      <Page size="A5" style={s.page}>
        ...
        <View style={s.table}>
          <View style={s.tableRow}>
            ...
            <Text style={[s.headerCell, s.colAmount]}>
              {t("จำนวนเงิน") + "\n" + t("(บาท)")}
            </Text>
          </View>
          ...
          {ITEMS.map((item, idx) => {
            ...
            return (
              <View style={s.tableRow} key={item.key}>
                <Text style={[s.cell, s.colMeter]}>
                  {t(idx <= 3 ? (meterCurr[idx] ?? "") : "")}
                </Text>
                <Text style={[s.cell, s.colMeter]}>
                  {t(idx <= 3 ? (meterPrev[idx] ?? "") : "")}
                </Text>
                <Text style={[s.cell, s.colMeter]}>
                  {t(idx <= 3 ? (meterUsed[idx] ?? "") : "")}
                </Text>
                ...
                <Text style={[s.cell, s.colAmount]}>
                  {entry?.checked && entry.amount
                    ? t(fmt(parseFloat(entry.amount)))
                    : ""}
                </Text>
              </View>
            );
          ...
          <View style={s.tableRow}>
            <Text style={[s.totalCell, { width: "39%", textAlign: "right" }]}>
              {t("รวมเป็นเงิน (บาท)")}
            </Text>
            <Text style={[s.totalCell, { width: "47%" }]}>{t(" ")}</Text>
            <Text style={[s.totalCell, s.colAmount]}>
              {total > 0 ? t(fmt(total)) : ""}
            </Text>
          </View>
        </View>
        ...
        {/* Note */}
        <Text style={s.noteRow}>
          {t("ผู้เช่าจะต้องชำระเงินไม่เกินวันที่ 1-5 ของทุกเดือน")}
        </Text>
        <View
          style={{
            flexDirection: "row",
        ...
        >
          ...
          <View style={{ width: "50%" }}>
            ...
            >
              <Text style={{ fontSize: 9, fontWeight: 700, width: 52 }}>
                {t("ธนาคาร :")}
              </Text>
...
function exportToExcel(data: ReceiptData) {
  ...
  const total = ITEMS_DEF.reduce((sum, item) => {
    const entry = items[item.key as ItemKey];
    return sum + (entry?.checked ? parseFloat(entry.amount || "0") : 0);
  }, 0);
